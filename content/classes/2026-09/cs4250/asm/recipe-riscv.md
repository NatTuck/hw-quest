...
This is a step-by-step process for translating a C-style function into working
RISC-V assembly.

We run this natively on the SG2000 board (a 64-bit RISC-V C906 core running
Linux), so there is no cross-compiler step unless you're working on your host.

## 1. Setup the Function

Start with the boilerplate. Every function needs a label and needs to be made
...
In RISC-V, there is no `push` or `pop`. You must manually move the stack
pointer (`sp`).

* **The 16-Byte Rule:** The stack pointer **must** stay aligned to 16 bytes.
* **Saving `ra`:** If your function calls another function, you **must** save
...
* **Long-lived variables:** Use "Saved" registers (`s0–s11`). These stay the
  same even after you call another function. (Remember: if you use an `s`
  register, you must save/restore it in the prologue/epilogue).
* **Temporary/Short-lived:** Use `t0–t6`. These are "Caller-saved," meaning if
  you call another function, their values might be destroyed.
...
Unlike x86, RISC-V cannot do math on memory. You must load values into
registers first.
...
To do an `if (a == b) { ... }`, you usually write the branch to **skip** the
...
**Note:** If you have values in `t` registers that you need *after* the call,
you must save them to the stack before the call and reload them after. This is
why using `s` registers for important variables is easier!
